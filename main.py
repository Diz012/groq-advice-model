...
def generate_quiz(mssv, mon_hoc, ly_do_rot):
    ...
    Hãy tạo đúng chính xác {num_questions} câu hỏi trắc nghiệm cho môn {mon_hoc}. 
    Yêu cầu:
    1. Trả về đúng {num_questions} đối tượng trong danh sách 'danh_sach_cau_hoi'.
    2. Mức độ: {level_questions}.
    3. Định dạng JSON nghiêm ngặt.
    4. Có thể tham khảo trên trang eduquiz.vn
    (Không được trả về ít hơn hoặc nhiều hơn {num_questions} câu). 
